...
#    if [ `${FSLDIR}/bin/imtest $g` -eq 0 ]; then echo "not an image file: $g"; exit 1; fi
    ...
    echo "/group/plasticity/Projects/JT_Semantomotor/pipeline/fsl_reg_agt $g $T $totarget $REG -fnirt $FNIRTCNF -flirt \"$FLIRTOPS\";" >> $queuefile
    echo "/group/plasticity/Projects/JT_Semantomotor/pipeline/fsl_reg_ag $g $T $totarget $REG -fnirt $FNIRTCNF -flirt \"$FLIRTOPS\";"
